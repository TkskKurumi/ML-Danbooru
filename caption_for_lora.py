...
def make_args():
    ...
    parser.add_argument('--trigger_words', type=str, default='')
...
if __name__ == '__main__':
    ...
    for f in tqdm(files):
        ...
        tarname = basename+".caption"
        if(os.path.exists(tarname)):
            with open(tarname, "r") as f:
                tags = f.read().split(",")
        else:
            cls_list = demo.infer(f)
            cls_list.sort(reverse=True, key=lambda x: x[1])
            tags = [name for name, prob in cls_list]
            if(args.trigger_words):
                tags.extend(args.trigger_words.split(","))
